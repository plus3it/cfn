...
$TestedSessionHosts = Test-RetryNetConnection -ComputerName $SessionHosts.SessionHost

if ($TestedSessionHosts.StaleComputers) {
    Clear-RDSessionHost -SessionHost ($SessionHosts | Where-Object { $_.SessionHost -in $TestedSessionHosts.StaleComputers}) -ConnectionBroker $ConnectionBroker
}
...
$ValidAcl = Edit-AclIdentityReference -Acl $Acl -IdentityReference $IdentityReferences

if ($ValidAcl) {
    Invoke-RetryCommand -Command Set-Acl -ArgList @{Path=$UpdPath; AclObject=$ValidAcl} -CheckExpression '$?'
}
